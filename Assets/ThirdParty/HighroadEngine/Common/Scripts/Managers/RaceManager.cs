...
namespace MoreMountains.HighroadEngine
{
    ...
    public class RaceManager : MonoBehaviour
    {
        ...
        public UnityAction<List<BaseController>> OnShowEndGameData;
        ...
        public virtual void InitRespawnButton(GameObject vehicle)
        {
            if (RespawnButton != null)
            {
                ...
                SolidController solidController = vehicle.GetComponent<SolidController>();
                if (solidController != null)
                {
                    RespawnButton.onClick.AddListener(solidController.Respawn);
                ...
                {
                    RespawnButton.onClick.AddListener(carController.Respawn);
                    RespawnButton.gameObject.SetActive(true);
                }
            }
        }
        ...
        protected virtual void ShowFinalRanking(List<BaseController> playersRank)
        {
            ...
            for (int i = 0; i < playersRank.Count; i++)
            {
                ...
                {
                    RankItems[i].SetName(playersRank[i].name);
                ...
                if (playersRank[i].name == "Me")
                {
                    LocalDataPlayer.Instance.AddGoldByRank(i);
                }
            }
            
            if (playersRank[0].name == "Me")
            {
                string nextMap = LocalDataPlayer.Instance.GetNextMapID();
                if (nextMap != string.Empty)
                {
                    LocalDataPlayer.Instance.AddMap(nextMap);
                }
            }
